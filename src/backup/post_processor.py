...
class PostProcessor:
    ...
    def _get_user_prompt_from_influxdb(self, session_id: str, agent_type: str) -> Optional[str]:
        """Extract user prompt from InfluxDB data for the given session."""
        try:
            if agent_type == 'claude':
                # Query Claude UserPromptSubmit events
                query = f'''
                from(bucket: "{self.config['INFLUXDB_BUCKET']}")
                |> range(start: -{self.time_window_hours}h)
                |> filter(fn: (r) => r._measurement == "claude_code_events")
                |> filter(fn: (r) => r.session_id == "{session_id}")
                |> filter(fn: (r) => r.hook_event_type == "UserPromptSubmit")
                    ...
                    for record in table.records:
                        ...
                        if payload_str:
                            try:
                                ...
                                if prompt:
                                    # Return the EXACT prompt as sent to the agent (with eval markers)
                                    return prompt
                            except json.JSONDecodeError:
                                continue
                
            elif agent_type == 'opencode':
                # For OpenCode, try to get from message events (though they may be empty)
                # This is a placeholder - OpenCode doesn't seem to capture user prompts in our current setup
                pass
                
        except Exception as e:
            print(f"   ⚠️ Failed to extract user prompt from InfluxDB: {e}")
    ...
    def process_single_session(self, session: MonitoringSession, session_number: int) -> Dict[str, Any]:
        ...
        with open(log_file, 'w', encoding='utf-8') as f:
            ...
            # Extract and display user prompt from InfluxDB
            user_prompt = self._get_user_prompt_from_influxdb(session.session_id, session.agent_type)
            if user_prompt:
                f.write("=== USER PROMPT ===\n")
                    ...
                    if payload:
                        ...
                        for key, value in payload.items():
                            if key == 'transcript_path':
                                ...
                                f.write(f"Tool Response: {json.dumps(value, indent=2)}\n")
                            elif key == 'prompt' and event.get('type') == 'UserPromptSubmit':
                                f.write("--- USER PROMPT ---\n")
                                f.write(f"Original Prompt: {value}\n")
                            else:
                                f.write(f"{key}: {value}\n")
